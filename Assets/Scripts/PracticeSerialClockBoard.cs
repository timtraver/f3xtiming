...
public class PracticeSerialClockBoard : MonoBehaviour
{
    ...
    public PracticeClockUpdate topClock;
    ...
    void Update()
    {
        if (currentSerialPort != "None")
        {
            if( queueControl.queueTimerRunning)
            {
                if( queueControl.clockTimerRunning)
                {
                    ...
                    if (clockSec != secondsOld || clockMin != minutesOld)
                    {
                        ...
                        SendSerialData(string.Format("{0:00}{1:00}", clockMin, clockSec), type, queueControl.playList[queueControl.currentQueueEntry].round_number, queueControl.playList[queueControl.currentQueueEntry].group);
                        secondsOld = clockSec;
                        minutesOld = clockMin;
                    }
                }
            ...
            {
                // Send clock time when timer is idle if thepref is selected
